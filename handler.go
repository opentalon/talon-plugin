...
func (h *handler) Capabilities() plugin.CapabilitiesMsg {
	return plugin.CapabilitiesMsg{
		// Capability name is `talon`, matching the convention of
		// sibling plugins (repo `<name>-plugin`, capability `<name>` —
		// e.g. hello-world-plugin → "hello-world"). The LLM sees
		// `talon.execute_workflow`; operators should use `talon:` as
		// the config-map key so the reverse-proxy path aligns
		// (`/talon/*`).
		Name:        "talon",
